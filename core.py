...
Owns parsing, validated AI output, SQLite persistence, exports, and the
...
def _database() -> Iterator[sqlite3.Connection]:
    """Open a transaction and always release the Windows file handle."""
    connection = _open_db()
    try:
        with connection:
            yield connection
...
def _migrate_legacy_csv(connection: sqlite3.Connection) -> None:
